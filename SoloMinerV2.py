...
print(Back.BLUE, Fore.WHITE, "BTC WALLET:", Fore.BLACK, address, Style.RESET_ALL)
...
def handler(signal_received, frame):
    ...
    ctx.fShutdown = True
    print(
        Fore.MAGENTA, "[", timer(), "]", Fore.YELLOW, "Terminating Miner, Please Wait.."
    )
...
def logg(msg):
    # basic logging
    logging.basicConfig(
        level=logging.INFO, filename="miner.log", format="%(asctime)s %(message)s"
    )  # include timestamp
    logging.info(msg)
...
def bitcoin_miner(t, restarted=False):
    ...
    # little endian
    merkle_root = "".join(
        [merkle_root[i] + merkle_root[i + 1] for i in range(0, len(merkle_root), 2)][
            ::-1
        ]
    )
    ...
    while True:
        ...
        if ctx.prevhash != ctx.updatedPrevHash:
            ...
            logg(
                "[*] Best difficulty will trying to solve block {} was {}".format(
                    work_on + 1, ctx.nHeightDiff[work_on + 1]
                )
            )
            print(
                Fore.MAGENTA,
                ...
                " {} ",
                Fore.BLUE,
                "was {}".format(work_on + 1, ctx.nHeightDiff[work_on + 1]),
            )
            ...
            print(
                ...
                "]",
                Fore.BLUE,
                "Bitcoin Miner Restart Now...",
                Style.RESET_ALL,
            )
            continue

        nonce = hex(random.randint(0, 2**32 - 1))[2:].zfill(
            8
        )  # nNonce   #hex(int(nonce,16)+1)[2:]
        blockheader = (
            ctx.version
            + ctx.prevhash
            + merkle_root
        ...
        hash = hashlib.sha256(
            hashlib.sha256(binascii.unhexlify(blockheader)).digest()
        ).digest()
...
def block_listener(t):
    ...
    sock.sendall(
        b'{"params": ["'
        + address.encode()
        + b'", "password"], "id": 2, "method": "mining.authorize"}\n'
    )
    response = b""
...
class NewSubscribeThread(ExitedThread):
    ...
    def thread_new_block(self, arg):
        ...
        try:
            ...
            print(
                Fore.MAGENTA, "[", timer(), "]", Fore.YELLOW, "[*] Subscribe thread()"
            )
            logg(e)
            traceback.print_exc()
        ctx.listfThreadRunning[self.n] = False
...
def StartMining():
    ...
    print(
        Fore.MAGENTA, "[", timer(), "]", Fore.GREEN, "[*] Bitcoin Miner Thread Started"
    )
    print(
        Fore.BLUE,
        "--------------~~( ",
